//! RustRTOS - ESP32-S3高性能实时操作系统库
//! 本库提供以下核心功能:
//! - 多优先级任务调度(基于Embassy + esp-rtos)
//! - 双核SMP支持
//! - PSRAM内存管理
//! - 内存池分配器
//! - DMA缓冲区管理
...
//! - 高性能环形缓冲区
//! - 条件编译日志系统
//! - WiFi网络连接(可选, 需启用wifi feature)
//! - BLE低功耗蓝牙(可选, 需启用ble feature)
//! - TCP/IP网络栈(可选, 需启用network feature)
...
pub mod config {
    ...
    /// 系统Tick频率(Hz) - Embassy配置为1MHz
    pub const TICK_FREQ_HZ: u32 = 1_000_000;

    /// 高优先级中断等级(ESP32-S3 Xtensa最高为Priority3)
