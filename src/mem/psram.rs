//! PSRAM管理模块
//! 提供ESP32-S3外部PSRAM (8MB)的初始化和分配功能。
//! 支持自动缓存策略选择，默认使用缓存模式以获得最佳性能。
//! 缓存策略
//! - CacheMode::Auto: 根据分配用途自动选择(默认缓存)
//! - CacheMode::Cached: 使用CPU缓存，适合频繁随机访问
//! - CacheMode::Direct: 直接访问，适合顺序大块传输
//! 注意事项
//! - PSRAM延迟约100ns，DRAM约10ns
//! - 缓存模式下需要注意DMA的cache一致性
//! - 非实时任务的大型缓冲区推荐使用PSRAM
...
pub enum CacheMode {
    ...
    /// 默认使用缓存模式
    Auto,
    /// 缓存模式：CPU L1/L2缓存，适合随机访问
    Cached,
    /// 直接模式：绕过缓存，适合DMA和大块顺序访问
    Direct,
}
...
/// 初始化PSRAM
/// esp-hal 1.0在启用psram feature时会自动初始化PSRAM。
/// 此函数用于获取PSRAM的基地址和大小。
/// Safety
/// 应该在系统启动时调用一次。
pub fn init() -> Result<PsramInfo, PsramError> {
    ...
    // esp-hal 1.0 with psram feature会自动初始化
    // PSRAM地址范围: 0x3C000000 - 0x3C7FFFFF (8MB)
    // 注意: 实际基地址和大小需要从esp-hal获取

    // 使用esp-hal提供的PSRAM信息
    // 默认ESP32-S3-N16R8配置: 8MB Octal PSRAM
    let base = 0x3C00_0000_usize; // PSRAM映射基地址
...
/// PSRAM分配的智能指针
/// 类似Box<T>，但数据存储在PSRAM中。
/// 注意: 当前实现使用bump allocator，不支持释放单个分配。
...
// 注意: 当前bump allocator不支持释放，所以不实现Drop
