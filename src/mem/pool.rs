//! 内存池分配器
//! 提供固定大小块的高效内存分配，支持DRAM和PSRAM后端。
//! 使用无锁位图追踪实现O(1)分配和释放。
//! 特性
//! - 零拷贝: 分配的内存可以直接使用
//! - 无锁: 使用原子操作实现线程安全
//! - 确定性: O(1)时间复杂度的分配和释放
//! - 灵活后端: 支持DRAM (低延迟)和PSRAM (大容量)
//! 示例
...
// Xtensa不原生支持AtomicU64，使用portable_atomic
...
pub enum Backend {
    /// DRAM: 内部RAM，低延迟(~10ns)
    Dram = 0,
    /// PSRAM (缓存模式): 外部RAM，中等延迟
    PsramCached = 1,
    /// PSRAM (直接模式): 外部RAM，适合DMA
    PsramDirect = 2,
    /// 自动选择: 根据大小和用途决定
    Auto = 3,
}
...
// Safety: PoolBox的安全性继承自MemoryPool
