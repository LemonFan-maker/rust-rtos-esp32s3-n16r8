//! 双核调度模块
//! 提供ESP32-S3双核(Core0 + Core1)的任务调度支持。
//! 支持两种模式:
//! - 自动分配: 按任务类型自动分配核心
//! - 手动分配: 用户显式指定运行核心
//! 核心分配策略
...
//! - Core1 (APP_CPU): IO密集型、传感器采样、计算密集型
//! 示例
...
impl Core1 {
    ...
    /// 启动Core1
    /// 使用esp-rtos的start_second_core函数启动第二个核心。
    /// 参数
    /// - cpu_ctrl: CPU控制外设
    /// - sw_int: 软件中断(用于核间通信)
    /// - stack: Core1的栈空间
...
// Safety: IpcChannel使用SPSC队列，一个核心发送，另一个核心接收
