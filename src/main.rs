//! RustRTOS - ESP32-S3高性能实时操作系统
//! 基于esp-hal 1.0和esp-rtos，采用混合调度策略：
//! - 协作式async/await任务调度(Embassy)
//! - 中断驱动的高优先级任务抢占(InterruptExecutor)
//! - 双核支持(Core0 + Core1)
//! 硬件目标: ESP32-S3-N16R8 (双核Xtensa LX7 @ 240MHz, 16MB Flash, 8MB PSRAM)
...
async fn main(low_prio_spawner: Spawner) {
    ...
    // 2. GPIO初始化(esp-hal 1.0新API)
    // 板载LED (根据实际硬件调整引脚)
    let led = Output::new(peripherals.GPIO2, Level::Low, OutputConfig::default());

    // 3.定时器初始化(Embassy时间驱动)
    ...
    // 5.初始化esp-rtos调度器
    // 注意: Xtensa架构只需要timer参数，RISC-V才需要software_interrupt
    esp_rtos::start(timg0.timer0);

    log_info!("esp-rtos scheduler initialized");

    // 6.配置高优先级执行器(Priority3是ESP32-S3最高可用)
